...
LLMs are renowned for writing defensive code. And a lot of that defensive code is genuinely pointless, but there is a chunk of it that, in my view, is a sign your types are not constraining enough. For example, suppose we want to store a 1-10 grade value on a student type, which is updatable. Asking GPT it created the following,
...
fn update_grade(student: &mut Student, new_grade: Score) {
  student.grade = new_grade;
}
```
...
All in all, I think LLM-written defensive code is often a syphtom of unrestricted types. We can do a better job and achieve better outcomes from LLM-written code by constraining our types to make sure we're encoding what we actually care about.
